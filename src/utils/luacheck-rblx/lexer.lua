...
local function is_alpha(b)
	return (BYTE_a <= b and b <= BYTE_z) or (BYTE_A <= b and b <= BYTE_Z) or b == BYTE_LDASH
end
...
function lexer.next_token(state)
	...
	if relative_error_offset then
		...
		local error_message = token_value
			.. " "
			.. lexer.get_quoted_substring_or_line(state, state.line, error_offset, error_end_offset)
