local core_utils = require(script.Parent.core_utils)
...
local function passes_rules_filter(rules, code, name)
	...
	for _, rule in ipairs(rules) do
		...
		for _, pattern in ipairs(rule[1]) do
			...
			if (matches_code and matches_name ~= false) or (matches_name and matches_code ~= false) then
				matches_one = true
			end
...
local function passes_filter(normalized_options, warning)
	...
	elseif warning.code:find("^1[14]") then
		if
			warning.indirect
			and get_field_status(normalized_options, warning, warning.previous_indexing_len) == "undefined"
		then
			return false
		end
...
local function filter_not_global_related_in_file(check_result, options_normalizer, stds, option_stack)
	...
	for line in ipairs(check_result.line_lengths) do
		next_inline_option_index =
			update_option_stack_for_new_line(check_result, stds, option_stack, line, next_inline_option_index)
		...
		if not check_result.fatal then
			if check_result.warnings[1] and check_result.warnings[1].code == "011" then
				...
				filter_not_global_related_in_file(
					check_result,
					caching_options_normalizer,
					stds,
					base_file_option_stack
				)
			end
		end
	end
...
local function filter_global_related_in_file(check_result, globally_defined, globally_used, locally_defined)
	for _, warning in ipairs(check_result.warnings) do
		if warning.code:find("^1") then
			...
			if warning then
				if
					warning.code:find("^11[12]")
					and not warning.module
					and get_field_status(normalized_options, warning) == "read_only"
				then
					warning.code = "12" .. warning.code:sub(3, 3)
				elseif
					warning.code:find("^11[23]") and get_field_status(normalized_options, warning, 1) ~= "undefined"
				then
