local check_state = require(script.Parent.check_state)
local core_utils = require(script.Parent.core_utils)
local parse_inline_options = require(script.Parent.stages.parse_inline_options)
			...
			if not fields_set[field] then
				error(
					"Unknown field "
						.. field
						.. " in "
						.. (per_code_fields and "issue with code " .. t.code or "inline option table"),
					0
				)
			end
		end
